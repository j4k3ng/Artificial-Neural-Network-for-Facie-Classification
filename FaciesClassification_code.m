...
%% SET UP INPUT AND TARGET AND RESET ARRAY
x = input_data;
t = target_data;
%defining ranges
...
%                 [net] = Divideind(net,w,x,t,WELLS_data);
            ...
            %voting results
            [y,yind,STOREy,STOREtr,Ystore,YINDstore] = Vote(Ystore,YINDstore,tind,STOREy,STOREtr,tr,w,layers,hidden);
%           [y,yind,STOREy,STOREtr,Ystore,YINDstore,ETest] = VoteTheBest(ETest,Ystore,YINDstore,STOREy,STOREtr,tr,w,layers,hidden);
            % Calculate Accuracy and F1
            [AccuracyM1_test,AccuracyM1_tot] = Accuracy_M1(AccuracyM1_test,AccuracyM1_tot,tind,yind,hidden,tr,w,layers);
            [AccuracyM2_test,AccuracyM2_tot] = Accuracy_M2(AccuracyM2_test,AccuracyM2_tot,tind,yind,hidden,tr,w,layers);
            [AccuracyM3_test,AccuracyM3_tot] = Accuracy_M3(AccuracyM3_test,AccuracyM3_tot,tind,yind,hidden,tr,w,layers);
            
            [F1M1avg,F1M1,precision_M1,recall_M1] = F1_M1(F1M1avg,facies,w,tr,yind,tind,layers,hidden);
            [F1M2avg,F1M2,precision_M2,recall_M2] = F1_M2(F1M2avg,facies,w,tr,yind,tind,layers,hidden);
            [F1M3avg,precision_M3,recall_M3] = F1_M3(F1M3avg,w,tr,yind,tind,layers,hidden);
            
            [F1M1avg_tot,F1M1_tot,precision_M1_tot,recall_M1_tot] = F1_M1_tot(F1M1avg_tot,facies,w,tr,yind,tind,layers,hidden);
            [F1M2avg_tot,F1M2_tot,precision_M2_tot,recall_M2_tot] = F1_M2_tot(F1M2avg_tot,facies,w,tr,yind,tind,layers,hidden);
            [F1M3avg_tot,precision_M3_tot,recall_M3_tot] = F1_M3_tot(F1M3avg_tot,w,tr,yind,tind,layers,hidden);
        end
...
save FINAL_128_10tries_NOVOTE_NOEARLYSTOPPING.mat
%% FUNCTIONS
...
function [net] = Divideind(net,w,x,t,WELLS_data)
%% ORDER DATA
training_data = [];  
training_index = [];
training_find = [];
...
function [AccuracyM1_test,AccuracyM1_tot] = Accuracy_M1(AccuracyM1_test,AccuracyM1_tot,tind,yind,hidden,tr,w,layers)
AccuracyM1_tot{w,layers}(hidden,1) = 1-(sum(tind ~= yind)/numel(tind)); 
AccuracyM1_test{w,layers}(hidden,1) = 1-(sum(tind(:,tr.testInd) ~= yind(:,tr.testInd))/numel(tind(:,tr.testInd)));
end
function [F1M1avg,F1M1,precision_M1,recall_M1] = F1_M1(F1M1avg,facies,w,tr,yind,tind,layers,hidden)
%initialize
...
%average F1
F1M1avg{w,layers}(hidden,1) = sum(facies(w,:).* F1M1(1,:)) / sum(facies(w,:)); 
end
function [AccuracyM2_test,AccuracyM2_tot] = Accuracy_M2(AccuracyM2_test,AccuracyM2_tot,tind,yind,hidden,tr,w,layers) 
...
AccuracyM2_test{w,layers}(hidden,1) = 1-(FAIL / length(TT));
%tot
FAIL = 0;
for i = 1:length(tind)
...
AccuracyM2_tot{w,layers}(hidden,1) = 1-(FAIL / length(tind));
end
function [F1M2avg,F1M2,precision_M2,recall_M2] = F1_M2(F1M2avg,facies,w,tr,yind,tind,layers,hidden)
Adj = {[2]; [1,3]; [2]; [5]; [4,6]; [5,7,8]; [6,8]; [6,7,9]; [7,8]};
%initialize
TrueNegative = zeros(9,length(tind));
FalseNegative = zeros(9,length(tind));
...
%average F1
F1M2avg{w,layers}(hidden,1) = sum(facies(w,:).* F1M2(1,:)) / sum(facies(w,:)); 
end
function [AccuracyM3_test,AccuracyM3_tot] = Accuracy_M3(AccuracyM3_test,AccuracyM3_tot,tind,yind,hidden,tr,w,layers)
PayFacies = [6,7,8,9];
%test set
PayFaciesCount = 0;
FAIL = 0;
...
AccuracyM3_test{w,layers}(hidden,1) = 1 - (FAIL / PayFaciesCount);
%tot set
PayFaciesCount = 0;
FAIL = 0;
for i = 1:length(tind)
...
AccuracyM3_tot{w,layers}(hidden,1) = 1-(FAIL / PayFaciesCount);
end
function [F1M3avg,precision_M3,recall_M3] = F1_M3(F1M3avg,w,tr,yind,tind,layers,hidden)
PayFacies = [6,7,8,9];
%initialize
...
F1M3avg{w,layers}(hidden,1) = 2*precision_M3*recall_M3 / (precision_M3 + recall_M3) ;
end 
function [y,yind,STOREy,STOREtr,Ystore,YINDstore] = Vote(Ystore,YINDstore,tind,STOREy,STOREtr,tr,w,layers,hidden)
...
function [F1M1avg_tot,F1M1_tot,precision_M1_tot,recall_M1_tot] = F1_M1_tot(F1M1avg_tot,facies,w,tr,yind,tind,layers,hidden)
%initialize
TrueNegative = zeros(9,length(tind));
FalseNegative = zeros(9,length(tind));
TruePositive = zeros(9,length(tind));
...
%average F1
F1M1avg_tot{w,layers}(hidden,1) = sum(facies(w,:).* F1M1_tot(1,:)) / sum(facies(w,:)); 
end
function [F1M2avg_tot,F1M2_tot,precision_M2_tot,recall_M2_tot] = F1_M2_tot(F1M2avg_tot,facies,w,tr,yind,tind,layers,hidden)
Adj = {[2]; [1,3]; [2]; [5]; [4,6]; [5,7,8]; [6,8]; [6,7,9]; [7,8]};
%initialize
TrueNegative = zeros(9,length(tind));
FalseNegative = zeros(9,length(tind));
...
%average F1
F1M2avg_tot{w,layers}(hidden,1) = sum(facies(w,:).* F1M2_tot(1,:)) / sum(facies(w,:)); 
end
function [F1M3avg_tot,precision_M3_tot,recall_M3_tot] = F1_M3_tot(F1M3avg_tot,w,tr,yind,tind,layers,hidden)
PayFacies = [6,7,8,9];
%initialize
TrueNegative = zeros(1,length(tind));
FalseNegative = zeros(1,length(tind));
...
F1M3avg_tot{w,layers}(hidden,1) = 2*precision_M3_tot*recall_M3_tot / (precision_M3_tot + recall_M3_tot) ;
end 
